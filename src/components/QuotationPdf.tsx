...
export async function printQuotationPdf(quotation: Quotation) {
...
export async function downloadQuotationPdf(quotation: Quotation) {
  ...
  try {
    ...
    if (!pageEl) throw new Error("Quotation page element not found");
    ...
    const safe =
      ...
      (quotation.number || "quotation").replace(/[<>:"/\\|?*\x00-\x1f]/g, "_").trim() ||
      "quotation";
    pdf.save(`${safe}.pdf`);
  } finally {
    iframe.remove();
...
function buildQuotationPrintHtml(quotation: Quotation): string {
  // Reuse invoice totals math — Quotation shares the required fields.
  const totals = computeTotals(quotation as unknown as Invoice);
  ...
  const logoSrc = quotation.company.logoDataUrl || defaultLogoAsset.url;
  const watermarkSrc = quotation.company.watermarkDataUrl || defaultWatermarkAsset.url;
  ...
  const signature = quotation.company.signatureDataUrl
    ? `<img class="signature" src="${escapeHtml(quotation.company.signatureDataUrl)}" alt="Signature" />`
    : `<div class="signature-spacer"></div>`;

  const displayNumber = quotation.number;

  const infoRows: Array<{ label: string; value: string }> = [
    { label: "Quotation No", value: displayNumber },
    { label: "Date", value: formatDate(quotation.date) },
  ];
  if (quotation.validityDate) {
    infoRows.push({ label: "Valid Until", value: formatDate(quotation.validityDate) });
  }
  if (quotation.salesPerson) infoRows.push({ label: "Sales Person", value: quotation.salesPerson });
  if (quotation.convertedInvoiceNumber) {
    infoRows.push({ label: "Converted Invoice", value: quotation.convertedInvoiceNumber });
  }
  if (quotation.transportMode)
    infoRows.push({ label: "Transport", value: quotation.transportMode });
  if (quotation.packages) infoRows.push({ label: "Packages", value: String(quotation.packages) });
  if (quotation.weight) infoRows.push({ label: "Weight", value: String(quotation.weight) });
  ...
  const termsHtml = quotation.terms
    ? `<div class="terms">
        ...
        <pre class="terms-body">${escapeHtml(quotation.terms)}</pre>
      </div>`
    : "";
...
<head>
  ...
  <title>${escapeHtml(displayNumber || "Quotation")}</title>
  <style>
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; color-adjust: exact; }
    ...
    .terms { padding: 8px 12px; }
    .terms-title { color: #000; font-size: 10px; letter-spacing: 1px; font-weight: 700; }
    .terms-body { margin: 4px 0 0; padding: 8px 10px; background: #f7f4ef; border: 1px solid #e6ddcf; border-radius: 4px; font-family: Helvetica, Arial, sans-serif; font-size: 11px; color: #000; white-space: pre-wrap; }
    ...
    <section class="invoice-shell">
      ...
      <div class="top-header">
        ...
        <div class="top-header-center">
          <span class="tax-invoice-title">QUOTATION</span>
          <div class="company-name">${escapeHtml(quotation.company.name)}</div>
          <div class="company-tagline">${escapeHtml(quotation.company.companyTagline)}</div>
        </div>
        <div class="top-header-spacer" aria-hidden="true"></div>
      </div>
      <header class="header">
        <div class="company-info">
          <div class="meta">GSTIN: <strong>${escapeHtml(quotation.company.gstin)}</strong></div>
          <div class="meta">${escapeHtml(quotation.company.address)}</div>
          <div class="meta">Phone: ${escapeHtml(quotation.company.phone)}${quotation.company.phone2 ? ` / ${escapeHtml(quotation.company.phone2)}` : ""}${quotation.company.email ? ` · ${escapeHtml(quotation.company.email)}` : ""}</div>
        </div>
      </header>
      ...
      <div class="top-grid">
        ...
        <div class="box customer">
          ...
          <div class="customer-name">${escapeHtml(quotation.customer.name)}</div>
          ${quotation.customer.address ? `<div>${escapeHtml(quotation.customer.address)}</div>` : ""}
          <div>Phone: ${escapeHtml(quotation.customer.phone)}</div>
      ...
      <div class="summary-grid">
        <div class="words">
          ...
          ${quotation.notes ? `<div class="label">NOTES</div><p>${escapeHtml(quotation.notes)}</p>` : ""}
        </div>
        <div class="summary">
      ...
      <div class="bottom">
        <div class="bank">
          ...
          <div class="bank-body">
            <div class="bold">${escapeHtml(quotation.bank.bankName)}</div>
            <div>A/C No : ${escapeHtml(quotation.bank.accountNumber)}</div>
            <div>IFSC Code : ${escapeHtml(quotation.bank.ifsc)}</div>
            ${quotation.bank.branch ? `<div>Branch : ${escapeHtml(quotation.bank.branch)}</div>` : ""}
          </div>
        </div>
        <div class="signature-box">
          <div style="text-align:center;">This quotation is valid until the mentioned validity date.</div>
          ${signature}
          <div class="signature-line">Authorized Signature<br />For ${escapeHtml(quotation.company.name)}</div>
        </div>
      </div>
      </div>
    </section>
